...
- For custom providers, prepare chat and embedding API settings. Model names
  ...
  LiteLLM-native and do not require a base URL. Custom OpenAI-compatible APIs
  can use any model ID, including IDs such as `vendor/model`, with a `/v1` base
  URL. Do not include `/chat/completions` in the base URL.
